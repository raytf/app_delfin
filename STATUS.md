...
> Last updated: 2026-05-04 (Windows PowerShell helpers now live under `scripts/windows/`, including `download-ci-bridge.ps1` and `test-ci-bridge.ps1` for consuming CI-built `delfin-litert-bridge-windows-x64` artifacts plus the moved `setup-check.ps1`.)
...
| `scripts/setup-check.sh` / `scripts/windows/setup-check.ps1`                 | ✅     | Environment validation helpers for Unix shells and native Windows PowerShell, including LiteRT C++ bridge/model sanity checks on Windows. |
| `README.md`                                                                  | ✅     | User-facing setup guide. Native Windows LiteRT C++ section now documents both the local Bazel/MSVC build flow and the new Windows CI-artifact helper commands under `scripts/windows/`. |
...
| `scripts/windows/download-ci-bridge.ps1`                                   | ✅     | Native Windows helper that downloads `delfin-litert-bridge-windows-x64` from the latest successful `build-litert-cpp-bridge.yml` run (or a specified run) via `gh` and stages the exe + DLL into `bin/`. |
| `scripts/windows/test-ci-bridge.ps1`                                       | ✅     | Native Windows helper that verifies the downloaded bridge files, ensures `.env` points at them, downloads the `.litertlm` model when needed, starts `npm run dev:litert-cpp`, waits for `/health`, and can run `benchmark:litert-cpp`. |
