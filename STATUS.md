...
> Last updated: 2026-05-04 (LiteRT-LM C++ developer environment: Linux / macOS (arm64) / WSL2 is now the default for `npm run setup:litert-cpp`; native Windows local builds opt-in via `--native-windows`. Windows binaries for end-user distribution are produced by CI per `distribution-cicd-spec.md`).
...
| GitHub Actions matrix builds                          | ⚠️     | `.github/workflows/build-litert-cpp-bridge.yml` produces native bridge binaries (windows-x64, macos-arm64, linux-x64) on push / release / dispatch. Full electron-builder packaging matrix (`dist.yml`) still pending — tracked in `distribution-cicd-spec.md`. |
