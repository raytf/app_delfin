...
> Last updated: 2026-05-04 (Platform setup docs are now split into dedicated macOS / Linux / Windows guides under `docs/features/backend/`, and `README.md` links to each per-OS runbook.)
...
| `README.md`                                                                  | ✅     | User-facing setup guide. Native Windows LiteRT C++ section now documents both the local Bazel/MSVC build flow and the new Windows CI-artifact helper commands under `scripts/windows/`, including the `gh` prerequisite/install command. |
...
| `docs/features/backend/testing-guide-windows.md`                           | ✅     | Full Windows runbook covering WSL2, native CI-artifact validation, and native local build flows for the LiteRT C++ backend. |
