...
| [`features/native-windows-backend-research-spec.md`](features/native-windows-backend-research-spec.md)             | 🚧 In Progress | Sequential native Windows backend validation: LiteRT-LM C++ first, Foundry Local only as contingency. Native bridge builds and streams text; vision support is the remaining blocker. |
| [`features/litert-cpp-vision-spec.md`](features/litert-cpp-vision-spec.md)                                         | 🚧 In Progress | C++ bridge vision support and KV-cache session reuse. Fixes the `SendMessageAsync` call shape and vision backend, drops Turn 2+ TTFT from ~5,300 ms to ~450 ms. Gate 1 — awaiting approval. |
