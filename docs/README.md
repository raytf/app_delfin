...
| [`features/distribution/distribution-backend-migration-spec.md`](features/distribution/distribution-backend-migration-spec.md) | 🚧 In Progress | Wire the chosen backend (llama-server / LiteRT-CPP) into the Electron main process; off-Python TTS decision now points to Piper first, with native Kokoro as the follow-up quality track. Gate 1 — awaiting approval. |
