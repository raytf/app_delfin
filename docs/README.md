...
| File                                                                                                               | Status         | Description                                                                                                                                                          |
| ------------------------------------------------------------------------------------------------------------------ | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`features/distribution/desktop-distribution-mvp-spec.md`](features/distribution/desktop-distribution-mvp-spec.md) | 🚧 In Progress | Cross-platform distribution decision record. **Revised 2026-05-06**: LiteRT-LM C++ bridge on Windows x64, macOS arm64, and Linux x64; Python sidecar deprecated for distribution (developer fallback only). Gate 1 approved. |
| [`features/distribution/distribution-backend-migration-spec.md`](features/distribution/distribution-backend-migration-spec.md) | 🚧 In Progress | Wire `scripts/litert-cpp-proxy.mjs` into the packaged Electron runtime on all three OSes. **Revised 2026-05-06**: DM0 proxy wiring extended from Windows to macOS arm64 + Linux x64. Gate 1 — awaiting approval. |
| [`features/distribution/distribution-packaging-spec.md`](features/distribution/distribution-packaging-spec.md)     | 🚧 In Progress | electron-builder config, first-run download orchestration, NSIS/DMG/AppImage installers. **Revised 2026-05-06**: all three platforms bundle `delfin_litert_bridge`; DP3 PyInstaller track superseded. Gate 1 — awaiting approval. |
| [`features/distribution/distribution-cicd-spec.md`](features/distribution/distribution-cicd-spec.md)               | 🚧 In Progress | GitHub Actions matrix builds, distribution channel recommendations, code signing guide. **Revised 2026-05-06**: `dist.yml` downloads prebuilt bridge artifact for all three platforms; PyInstaller step removed. Track DC1a implemented as `build-litert-cpp-bridge.yml`. Full `dist.yml` matrix still awaiting approval. |
| [`features/distribution/version-constants-single-source-spec.md`](features/distribution/version-constants-single-source-spec.md) | 🚧 Gate 1 | Eliminate the `MODEL_REVISION` / `MODEL_REPO` / `MODEL_FILE` drift between `scripts/setup-litert-cpp.mjs` (dev setup) and `src/main/sidecar/assetManager.ts` (packaged app) by introducing `scripts/versions.mjs` as the single authoritative source, consumed via Vite `define` in the Electron build. |
