...
import { createWaveformBars, getWaveformActivityLevel, reduceFrequencyDataToWaveformBars, resolveWaveformPresentation, smoothWaveformBars, WAVEFORM_BAR_COUNT, } from './utils/waveformState';
...
export default function App() {
    ...
    const [overlayMode, setOverlayMode] = useState('expanded');
    const [minimizedVariant, setMinimizedVariant] = useState('compact');
    const [isMinimizedPromptComposing, setIsMinimizedPromptComposing] = useState(false);
    const [captureSourceLabel, setCaptureSourceLabel] = useState(null);
    const [sidecarStatus, setSidecarStatus] = useState({ connected: false });
    ...
    const latestResponseText = minimizedResponseMessageId === null
        ? null
    ...
    const audioSourceNodesRef = useRef(new Set());
    ...
    const assistantLevelAnimationFrameRef = useRef(null);
    const assistantWaveformBarsRef = useRef(createWaveformBars());
    const assistantWaveformFrequencyDataRef = useRef(null);
    const isAudioPlayingRef = useRef(false);
    const fallbackSpeechTimerRef = useRef(null);
    const minimizedVoiceCollapseTimerRef = useRef(null);
    const pendingVoiceWavRef = useRef(null);
    const lowerThresholdRef = useRef(null);
    const voiceEnabled = window.api.voiceEnabled;
    const ttsEnabled = window.api.ttsEnabled;
    const setAudioPlayingState = useCallback((nextValue) => {
    ...
    const startAssistantWaveformLoop = useCallback(() => {
        ...
        const sample = () => {
            ...
            if (assistantWaveformFrequencyDataRef.current === null ||
            ...
            const reducedBars = reduceFrequencyDataToWaveformBars(assistantWaveformFrequencyDataRef.current, WAVEFORM_BAR_COUNT);
    ...
    const syncOverlayStateFromMain = useCallback(async () => {
        try {
            ...
            setIsMinimizedPromptComposing(state.overlayMode === 'minimized' && state.minimizedVariant === 'prompt-input');
        }
        catch (error) {
    ...
    const submitVoiceTurn = useCallback((wavBase64) => {
    ...
    const { raiseThreshold, lowerThreshold, isListening, isMuted, isUserSpeaking, toggleMute, userAudioLevel, userWaveformBars, } = useVAD({
        ...
        onSpeechStart: () => {
            ...
            const isAssistantSpeaking = isAudioPlayingRef.current ||
    ...
    const handleSetMinimizedVariant = useCallback(async (variant) => {
        clearMinimizedVoiceCollapseTimer();
        try {
    ...
    async function handleStartSession(sessionName) {
    ...
    async function handleRestoreOverlay() {
        clearMinimizedVoiceCollapseTimer();
        setOverlayMode('expanded');
        clearLatestResponse();
        await window.api.restoreOverlay();
    }
    async function handleMinimizeOverlay() {
        clearLatestResponse();
        clearMinimizedVoiceCollapseTimer();
    ...
    function handleGoHomeFromEnded() {
        setEndedSessionData(null);
        setHomeView('landing');
        setSelectedPastSession(null);
        void window.api.clearEndedSession();
    }
    async function handleSubmitPrompt(text) {
        const trimmedText = text.trim();
    ...
    async function handleOpenPastSession(sessionId) {
    ...
    if (endedSessionData !== null) {
        return (_jsx(SessionEndedView, { duration: endedSessionData.duration, messageCount: endedSessionData.messageCount, onGoHome: handleGoHomeFromEnded, sessionName: endedSessionData.sessionName }));
    }
    if (sessionMode === 'active' && overlayMode === 'minimized') {
        return (_jsx(MinimizedSessionBar, { errorMessage: errorMessage, isAudioPlaying: isAudioPlaying, isSubmitting: isSubmitting, isMicListening: isListening, isMicMuted: isMuted, latestResponseText: latestResponseText, minimizedVariant: minimizedVariant, onAskAnother: () => {
            ...
            }, onToggleVadListening: toggleVadListening, showVoiceWaveform: showVoiceWaveform, vadListeningEnabled: vadListeningEnabled, waveformBars: waveformPresentation.bars, waveformState: waveformPresentation.state }));
    }
    if (sessionMode === 'active') {
        return (_jsx(ExpandedSessionView, { captureSourceLabel: captureSourceLabel, errorMessage: errorMessage, isAudioPlaying: isAudioPlaying, isSubmitting: isSubmitting, isMicListening: isListening, isMicMuted: isMuted, messages: messages, sessionName: activeSessionName, onMinimize: () => {
                void handleMinimizeOverlay();
            }, onStop: () => {
                void handleStopSession();
            }, onSubmitPrompt: (nextText) => {
                void handleSubmitPrompt(nextText);
            }, onToggleVadListening: toggleVadListening, showVoiceWaveform: showVoiceWaveform, sidecarStatus: sidecarStatus, vadListeningEnabled: vadListeningEnabled, waveformBars: waveformPresentation.bars, waveformState: waveformPresentation.state }));
    }
    if (sessionMode === 'home' && selectedPastSession !== null) {
        return (_jsx(PastSessionView, { messages: selectedPastSession.messages, onBack: () => {
                setSelectedPastSession(null);
            }, onDelete: () => {
    ...
    if (sessionMode === 'home' && homeView === 'all-sessions') {
        return (_jsx(AllSessionsPage, { onBack: () => {
                setHomeView('landing');
    ...
    return (_jsxs(_Fragment, { children: [_jsx(HomeScreen, { onDeleteSession: (sessionId) => {
                    void handleDeleteSession(sessionId);
                }, onSelectSession: (sessionId) => {
                    void handleOpenPastSession(sessionId);
                ...
                }, sessions: sessionHistory, userName: userName }), _jsx(UserNameModal, { isOpen: userName === null, onSave: (name) => {
                    setUserName(name);
                } })] }));
}
