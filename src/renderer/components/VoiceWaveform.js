...
import { resampleWaveformBars } from '../utils/waveformState';
const EXPANDED_BAR_COUNT = 28;
const COMPACT_BAR_COUNT = 18;
const BAR_GAP = 4;
export default function VoiceWaveform({ bars, className, compact = false, label, state }) {
    ...
    return (_jsx("div", { className: className, "data-state": state, children: _jsx("canvas", { "aria-label": accessibleLabel, className: `block w-full ${compact ? 'h-8' : 'h-14'}`, ref: canvasRef, role: "img" }) }));
}
function getWaveformMotionConfig(state) {
    switch (state) {
        case 'user':
            return {
