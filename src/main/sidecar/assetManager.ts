...
export type ModelAssetId = "litert-cpp-model" | "piper-bin" | "piper-voice";
...
const PIPER_VOICE_BASE = `https://huggingface.co/rhasspy/piper-voices/resolve/v1.0.0/en/en_US/hfc_female/medium`;
...
export async function downloadAssets(
  ...
  if (!existsSync(join(MODELS_DIR, "piper")))
    mkdirSync(join(MODELS_DIR, "piper"), { recursive: true });
    ...
    for (const id of assetsToDownload) {
      ...
      try {
        if (id === "piper-bin") {
          ...
          console.log(
            `[assetManager] Downloading ${id} from ${asset.url} to ${destPath}`,
          );
...
async function downloadAndExtractPiperBin(
  ...
  console.log(
    `[assetManager] Downloading piper-bin from ${url} to ${archivePath}`,
  );
  ...
  if (isWin) {
    ...
    if (result.status !== 0) {
      throw new Error(
        `Piper extraction failed: ${result.stderr || result.stdout}`,
      );
    }
  } else {
...
async function downloadPiperVoice(
  ...
  console.log(
    `[assetManager] Downloading piper voice config from ${configUrl}`,
  );
