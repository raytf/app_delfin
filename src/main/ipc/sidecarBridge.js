...
import { wsInterruptMessageSchema, wsOutboundMessageSchema, } from "../../shared/schemas";
...
export function registerSidecarBridge(options) {
    ...
    onSidecarMessage(async (message) => {
        ...
        try {
            switch (message.type) {
                case "token":
                    await options.sessionPersistence.appendAssistantToken(message.text ?? "");
                ...
                case "audio_chunk":
                    if (message.audio !== undefined) {
                        mainWindow.webContents.send(MAIN_TO_RENDERER_CHANNELS.SIDECAR_AUDIO_CHUNK, { audio: message.audio, index: message.index });
                    }
                    return;
                case "audio_end":
                    mainWindow.webContents.send(MAIN_TO_RENDERER_CHANNELS.SIDECAR_AUDIO_END, { ttsTime: message.tts_time ?? 0 });
                    return;
                case "done":
                    ...
                    return;
                case "error":
                    await options.sessionPersistence.failAssistantResponse(message.message ?? "Unknown error");
                    mainWindow.webContents.send(MAIN_TO_RENDERER_CHANNELS.SIDECAR_ERROR, {
            ...
            console.error("[sidecarBridge] Failed to persist sidecar message:", error);
        }
    });
