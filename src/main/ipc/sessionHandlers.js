...
export function registerSessionIpcHandlers(options) {
    ipcMain.handle(RENDERER_TO_MAIN_CHANNELS.SESSION_START, async (_event, request) => {
    ...
    ipcMain.handle(RENDERER_TO_MAIN_CHANNELS.SESSION_STOP, async (_event, request) => {
    ...
    ipcMain.handle(RENDERER_TO_MAIN_CHANNELS.SESSION_SUBMIT_PROMPT, async (_event, rawRequest) => {
    ...
    ipcMain.handle(RENDERER_TO_MAIN_CHANNELS.SESSION_GET_DETAIL, async (_event, request) => options.sessionPersistence.getSessionDetail(request.sessionId));
    ipcMain.handle(RENDERER_TO_MAIN_CHANNELS.SESSION_DELETE, async (_event, request) => options.sessionPersistence.deleteSession(request.sessionId));
    ipcMain.handle(RENDERER_TO_MAIN_CHANNELS.SESSION_GET_MESSAGE_IMAGE, async (_event, request) => options.sessionPersistence.getCaptureImageDataUrl(request.imagePath));
}
